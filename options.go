...
type Options struct {
	...
	OnReceive         ResultFn // callback on response receive
	CSV               bool
	Resume            bool
	Stream            bool
	Passive           bool
...
func ParseOptions() *Options {
	...
	flagSet.SetDescription(`port scanning tool written in Go that allows you to enumerate open ports for hosts in a fast and reliable manner.`)
	...
	flagSet.CreateGroup("config", "Configuration",
		...
		flagSet.StringSliceVarP(&options.IPVersion, "iv", "ip-version", []string{IPv4}, "ip version to scan of hostname (4,6) - (default 4)", goflags.NormalizedStringSliceOptions),
	...
	flagSet.CreateGroup("debug", "Debug",
		...
		flagSet.IntVarP(&options.MetricsPort, "metrics-port", "mp", 63636, "port to expose metrics on"),
	)
...
func (options *Options) shouldDiscoverHosts() bool {
	return (options.OnlyHostDiscovery || !options.SkipHostDiscovery) && !options.Passive && PkgRouter != nil
}
...
func (options *Options) shouldUseRawPackets() bool {
	return isOSSupported() && isPrivileged() && options.ScanType == SynScan && PkgRouter != nil
}
